...
class SVM:
    ...
    def run(self):
        time_create=timeit.timeit(self.create_SVM, number=10)
        self.create_SVM()     
        time_prediction=timeit.timeit(self.predire, number=10)        
        a=self.predire()
        print(f' Temps {time_create:.2f} seconds pour la création du SVM\n')
